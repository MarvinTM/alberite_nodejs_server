...
const connectionString =
  process.env.DATABASE_URL || "postgres://localhost:5432/alberite";
...
function moveActionToArchive(callback) {
  ...
  executeQuery(queryStr, null, function(results) {
    ...
    executeQuery(queryDelete, [action.index], function(results) {
      ...
      var queryInsertArchive =
        "INSERT INTO PAST_ACTIONS SELECT COALESCE(MAX(INDEX), 0)+1, $1, $2, $3, $4, $5 FROM PAST_ACTIONS";
...
function getMaxSystemAction(callback) {
  var queryStr =
    "SELECT COALESCE(MAX(index), 0)::integer as maxindex FROM ACTIONS";
  executeQuery(queryStr, null, function(results) {
    callback(results);
  });
}
...
function initiateSystemAction(pin, duration, callback, programmedAction) {
  getMaxSystemAction(function(maxIndex) {
    ...
    executeQuery(
      insertStr,
      [pin, duration, maxIndex[0].maxindex + 1, programmedAction],
      callback
    );
  });
}
...
function insertProgrammedAction(
  ...
  var insertStr =
    "INSERT INTO PROGRAMMED_ACTIONS SELECT $1, $2, coalesce(max(index), 0)+1, $3, $4 FROM PROGRAMMED_ACTIONS";
  executeQuery(
    insertStr,
    [phase, time, frequency, hour],
    callback,
    errorCallback
  );
}
...
function insertUser(username, password, callback) {
  console.log(
    "Registering username: " + username + " with password: " + password
  );
  auth.hash(password, function(err, hashed) {
    ...
    executeQuery(
      queryInsertUser,
      [username, hashed.hash, hashed.salt],
      function(results) {
        callback();
      }
    );
...
app.get("/loginfo", function(req, res) {
  var queryStr =
    "SELECT id, message, messagedate, type, externalip FROM loginfo ORDER BY messagedate DESC LIMIT 20";
  executeQuery(queryStr, null, function(results) {
...
app.get("/insertProgrammedAction", ensureAuthenticated, function(req, res) {
  ...
  insertProgrammedAction(
    programmedAction.phase,
    programmedAction.time,
    programmedAction.frequency,
    programmedAction.hour,
    function() {
      var response = { message: "Everything is ok", opCode: 0 };
      res.header("Access-Control-Allow-Origin", "*");
      res.json(response);
    ...
    function(error) {
      var response = {
        message: "Error al crear acción programada: " + error,
        opCode: 1
      };
      res.header("Access-Control-Allow-Origin", "*");
      res.json(response);
...
function processProgrammedAction(programmedActions, index, finishCallback) {
  ...
  if (actionShouldBeTriggered(programmedAction)) {
    ...
    findExistingProgrammedAction(programmedAction, function(results) {
      if (results.length === 0) {
        initiateSystemAction(
          programmedAction.phase,
          programmedAction.time,
          function() {
            console.log(
              "Programmed action triggered an action. Phase: " +
                programmedAction.phase
            );
            processProgrammedAction(
              programmedActions,
              index + 1,
              finishCallback
            );
...
function programmedActionsProcessor() {
  ...
  var queryProgrammedActions =
    "SELECT * FROM PROGRAMMED_ACTIONS ORDER BY INDEX ASC";
